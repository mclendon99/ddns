...
dnsdata=`curl -s -X GET -H "Authorization: sso-key ${GDAPIKEY}" "${GODADDYHOST}/v1/domains/${GODADDDYDOMAIN}/records/CNAME/${myhostname}"`
echo Response to CNAME query for ${myhostname}.${GODADDYDOMAIN} is $dnsdata
if [ "[]" = "$dnsdata" ] ; then
  echo "Adding a CNAME ${myhostname} record to domain ${GODADDDYDOMAIN}"
  resp=`curl -s -X PATCH  "${GODADDYHOST}/v1/domains/${GODADDDYDOMAIN}/records" -H "Authorization: sso-key ${GDAPIKEY}" -H "Content-Type: application/json" -d "[ {\"data\":\"@\",\"name\":\"${myhostname}\",\"port\":65535,\"ttl\":3600,\"type\":\"CNAME\"} ]"`
  echo Response to PATCH is: $resp
  echo "Added CNAME record for ${hostname}.${GODADDDYDOMAIN}"
else
